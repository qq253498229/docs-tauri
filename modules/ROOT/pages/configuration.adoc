...
配置文件在你的Tauri应用源目录(src-tauri)通过执行 xref:command_line_runner.adoc#init[tauri init]命令生成。
...
|`version`|`string`?|null|应用程序版本。它是一个符合语义化版本控制的版本号，或者是包含 `version` 字段的 `package.json` 文件的路径。
|===
...
|`x`|`number`?(format: `double`)|_null_|窗口左上角的水平位置。
|`y`|`number`?(format: `double`)|_null_|窗口左上角的垂直位置。
|`width`|`number`?(format: `double`)|`800`|窗口宽度。
|`height`|`number`?(format: `double`)|`600`|窗口高度。
|`minWidth`|`number`?(format: `double`)|_null_|窗口的最小宽度。
|`minHeight`|`number`?(format: `double`)|_null_|窗口的最小高度。
|`maxWidth`|`number`?(format: `double`)|_null_|窗口的最大宽度。
|`maxHeight`|`number`?(format: `double`)|_null_|窗口的最大高度。
|`resizable`|`boolean`|`true`|窗口是否允许调整大小。
...
|`args`|`array`?|_null_|命令的参数列表。
|`subcommands`|`object`?|_null_|命令的子命令列表。
|===
...
|`name`|`string`(required)|_null_|唯一参数名称。
...
|`multiple`|`boolean`|`false`|指定参数可能具有未知数量的多个值。如果没有其他设置，此参数只能出现一次。
...
|`icon`|`string[]`|[]|应用的图标
|`resources`|`array`?|_null_|应用程序需要打包的资源。每个资源是指向文件或目录的路径。支持使用通配符模式。
...
请注意，Tauri 会按照 "pass:[binary-name{-target-triple}{.system-extension}]" 的模式寻找特定于系统的二进制文件。
...
固定版本可以从 link:{webview2_url}[官方网站] 下载。必须将 `.cab` 文件解压到一个文件夹中，并在此字段中定义该文件夹路径。
...
这是配置的一个非常重要的部分，因为它帮助确保你的 WebView 是安全的。请参见 {csp_url}。
...
默认情况下，外部域无法访问 `pass:[window.__TAURI__]`，这意味着它们无法与 Rust 中定义的命令进行通信。这可以防止外部加载的恶意或受损网站在用户设备上执行命令的攻击。
...
|`domain`|`string`(required)|_null_|允许的域。
|`windows`|`string[]`(required)|_null_|此范围适用的窗口标签列表。
...
|`endpoints`|`array`?|_null_|更新程序端点。生产环境强制使用 TLS。

更新程序 URL 可以包含以下变量： +
- pass:[{{current_version}}]: 请求更新的应用程序版本 +
- pass:[{{target}}]: 操作系统名称（``linux``、`windows` 或 `darwin` 之一） +
- pass:[{{arch}}]: 机器的架构（``x86_64``、`i686`、`aarch64` 或 `armv7` 之一）

# 示例 +
- "link:https://my.cdn.com/latest.json[]": 一个原始 JSON 端点，返回最新版本和每个平台的下载链接。 +
- "link:https://updates.app.dev/pass:[{{target}}]?version=pass:[{{current_version}}]&arch=pass:[{{arch}}][]": 一个具有位置和查询字符串参数的专用 API。
...
|===
|名称|类型|默认|描述
|`iconPath`|`string`(required)|_null_|系统托盘上使用的默认图标的路径。
...
|`withGlobalTauri`|`boolean`|`false`|是否应该在 `pass:[window.__TAURI__]` 上注入 Tauri API。
|===
