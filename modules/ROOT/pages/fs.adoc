...
- BinaryFileContents: `Iterable`<``number``> | `ArrayLike`<``number``> | `link:{array_buffer_url}[ArrayBuffer]`
...
- readBinaryFile(`filePath`: `string`, `options?`: `<<fs-options,FsOptions>>`): `link:{promise_url}[Promise]`<``link:{uint8_url}[Uint8Array]``>
...
*返回*: `link:{promise_url}[Promise]`<``link:{uint8_url}[Uint8Array]``>
